...
class SheetPositionTestCase(unittest.TestCase):
    ...
    def test_point_text(self):
        position = pyoo.SheetPosition(10, 20)
        self.assertEqual("x=10, y=20", pyoo.text_type(position))
    ...
    def test_rectange_text(self):
        position = pyoo.SheetPosition(10, 20, 30, 40)
        self.assertEqual("x=10, y=20, width=30, height=40", pyoo.text_type(position))
        self.assertIsInstance(pyoo.text_type(position), pyoo.text_type)

    def test_replace_x(self):
        position = pyoo.SheetPosition(10, 20, 30, 40)
        self.assertEqual(
            "x=100, y=20, width=30, height=40", str(position.replace(x=100))
        )

    def test_replace_y(self):
        position = pyoo.SheetPosition(10, 20, 30, 40)
        self.assertEqual(
            "x=10, y=200, width=30, height=40", str(position.replace(y=200))
        )

    def test_replace_width(self):
        position = pyoo.SheetPosition(10, 20, 30, 40)
        self.assertEqual(
            "x=10, y=20, width=300, height=40", str(position.replace(width=300))
        )

    def test_replace_height(self):
        position = pyoo.SheetPosition(10, 20, 30, 40)
        self.assertEqual(
            "x=10, y=20, width=30, height=400", str(position.replace(height=400))
        )
...
class SheetAddressTestCase(unittest.TestCase):
    ...
    def test_cell_text(self):
        address = pyoo.SheetAddress(0, 1)
        self.assertEqual("$B$1", pyoo.text_type(address))
...
class CellRangeTestCase(BaseDocumentTestCase):
    ...
    def test_cell_text(self):
        cell = self.sheet[0, 0]
        self.assertEqual("$A$1", pyoo.text_type(cell))
        self.assertEqual("$A$1", pyoo.text_type(cell.address))
    ...
    def test_col_index_must_be_lt_col_count(self):
        with self.assertRaises(IndexError):
            # Since Libreoffice 7.3 max column number increased
            self.sheet[:, 16384]
    ...
    def test_get_default_tabular_range_from_sheet(self):
        cells = self.sheet[:, :]
        self.assertEqual("$A$1:$XFD$1048576", str(cells.address))

    def test_get_negative_tabular_range_from_sheet(self):
        cells = self.sheet[-20:-10, -6:-1]
        self.assertEqual("$XEY$1048557:$XFC$1048566", str(cells.address))
    ...
    def test_get_default_horizontal_range_from_sheet(self):
        row = self.sheet[10, :]
        self.assertEqual("$A$11:$XFD$11", str(row.address))

    def test_get_negative_horizontal_range_from_sheet(self):
        row = self.sheet[-10, -6:-1]
        self.assertEqual("$XEY$1048567:$XFC$1048567", str(row.address))
    ...
    def test_get_negative_vertical_range_from_sheet(self):
        ...
        self.assertEqual("$XFD$1048557:$XFD$1048566", str(column.address))
    ...
    def test_get_negative_cell_from_sheet(self):
        cell = self.sheet[-10, -1]
        self.assertEqual("$XFD$1048567", str(cell.address))
    ...
    def test_get_negative_cell_from_horizontal_range(self):
        row = self.sheet[10, :]
        cell = row[-1]
        self.assertEqual("$XFD$11", str(cell.address))
    ...
    def test_get_simple_tabular_range_from_sheet(self):
        ...
        self.assertEqual("$A$11:$XFD$20", str(cells.address))
    ...
    def test_datetime_cell_value(self):
        ...
        self.assertEqual(
            datetime.datetime(1985, 5, 6, 23, 55), cell.date.replace(microsecond=0)
        )
    ...
    def test_date_cell_value(self):
        ...
        # Should be almost equal because dates and times are represented as floats
        self.assertEqual(
            datetime.datetime(1985, 5, 6, 0, 0, 0), cell.date.replace(microsecond=0)
        )
    ...
    def test_object_cell_value(self):
        cell = self.sheet[0, 0]
        cell.value = MyObject()
        self.assertEqual("my object", cell.value)
    ...
    def test_object_cell_formula(self):
        ...
        self.assertEqual("my object", cell.value)
        self.assertEqual("my object", cell.formula)
...
class ChartsTestCase(BaseDocumentTestCase):
    ...
    def test_x_axis_title_given_as_object(self):
        with self.create_chart() as chart:
            ...
            self.assertEqual("my object", diagram.x_axis.title)
...
class SpreadsheetCollectionTestCase(BaseDocumentTestCase):
    ...
    def test_copy_sheet_with_index(self):
        sheet = self.document.sheets.copy(
            self.document.sheets[0].name, "Copied with index", 0
        )
...
class SheetTestCase(BaseDocumentTestCase):
    ...
    def test_sheet_text(self):
        ...
        sheet.name = "My Sheet"
        self.assertEqual("My Sheet", pyoo.text_type(sheet))
...
class NameGeneratorTestCase(unittest.TestCase):
    ...
    def test_names_with_casesensitive_chars_are_unique(self):
        get_name = pyoo.NameGenerator()
        get_name("Test Č")
        self.assertEqual("test č 2", get_name("test č"))
